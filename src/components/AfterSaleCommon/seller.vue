<template>
	<div class="wrap">
		<div class="applyInfo" v-if='refundInfo'>
			<el-row>
				<el-col :span='2'>
					申请时间：
				</el-col>
		...
		<div class="opera" v-if='refundInfo'>
			<div v-if='refundInfo.refund_state==="1"'>
				<el-button type='primary' size='small' @click='editApply'>
					修改申请
				</el-button>
				<el-button type='primary' size='small' @click='removeRefund'>
					撤销申请
				</el-button>
				<el-button type='primary' size='small' @click='msgBol=true'>
					留言
				</el-button>
			</div>
			<div v-if='refundInfo.refund_state==="3"'>
				<el-button type='primary' size='small' @click='expressBol=true'>
					填写退货单号
				</el-button>
				<el-button type='primary' size='small' @click='msgBol=true'>
					留言
		...
		<transition name='fade'>
			<div v-show='msgBol' class="mask">
				<div class="addGoodsInfo">
					<div class="addTitle">留言<i class="el-icon-close" @click='msgBol=false;'></i></div>
...
import {MessageBox,Message} from  'element-ui'
	export default{
		...
		methods:{
			...
			editApply(){
				let _this = this ;
				let refund_sn = _this.refundInfo.refund_sn ; 
